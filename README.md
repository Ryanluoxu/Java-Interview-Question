...
>- JDK is Java Development Kit including JRE and developer tools, like compiler and debugger - JRE is Java Runtime Environment where Java programs run on. Inside JRE basically it's JVM ( including class loader, Execution engine..) - For example, if we just want to run a Java program, JRE is enough. If we want to create a Java program, then we need JDK.
